...
export class IatRecorder {
  ...
  init() {
    ...
    try {
      if (!self.APPID || !self.APIKey || !self.APISecret) {
        ...
        // transAudioData.transcode({}, (event) => {
  ...
  recorderInit() {
    ...
    let getMediaSuccess = _ => {
      // 创建一个用于通过JavaScript直接处理音频
              console.log(8888880909)
      this.scriptProcessor = this.audioContext.createScriptProcessor(0, 1, 1);
      this.scriptProcessor.onaudioprocess = e => {
        console.log(888, this.status)
        if (this.status === 'ing') {
          // 多线程音频数据处理
          try {
            transAudioData.transcode(e.inputBuffer.getChannelData(0), (event) => {
            self.audioData.push(...event.data);
            })
  ...
  // 启动录音
  recorderStart() {
    console.log('recorderInit', this.audioContext)
    if (!this.audioContext) {
      this.recorderInit();
    } else {
      this.audioContext.resume();
